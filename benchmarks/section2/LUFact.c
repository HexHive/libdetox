...
void initialise(){
    ...
    a = (double**) malloc (sizeof(double*)* ldaa); 
    for (i=0; i<ldaa; i++){
      a[i] = (double*) malloc (sizeof(double)*lda); 
    }
    b = (double*) malloc (sizeof(double)*ldaa); 
    x = (double*) malloc (sizeof(double)*ldaa); 
    ipvt = (int*) malloc (sizeof(int)*ldaa); 
  ...
  void run(int sizein){
   ...
   ops = ops/1.0e06;     
   printf("Section2:LUFact:Kernel    %f (s)       %f      (Mflop/s)\n", kernel_time, (double) ops / kernel_time);  
