      ...
      */
      ...
      if (anyModRMOp(opcode) || (opcode->tableFlags&OPCODE_MODRM_ESC) || (opcode->tableFlags&OPCODE_FPU_ESC)) {
        ...
        if (opcode->tableFlags&OPCODE_MODRM_ESC) {
          ...
          PRINT_DEBUG("opcode table: %p -> %p (%p)\n", opcode, opcode->opcode.table, opcode->opcode.table + ((*cur & MASK_REG) >> SHIFT_REG));
...
unsigned int operandSize(unsigned int operandFlags, unsigned char prefix)
{
  ...
  if (prefix==PREFIX_OP_SZ_OVR && ((operandFlags&ADDRM_I)==ADDRM_I)) rprefix = prefix;
  if (prefix==PREFIX_ADDR_SZ_OVR &&
      (((operandFlags&ADDRM_A)==ADDRM_A) || ((operandFlags&ADDRM_J)==ADDRM_J) || ((operandFlags&ADDRM_O)==ADDRM_O)))
