...
/* is located int "fbt_opcode_tables.h" can not be included because of multiple occurences */
...
void fbt_init (ia32_opcode_t *opcode_table)
{
...
#endif
	/*
	 * if the user program specifies a custom opcode table we overwrite the default table
	 * overwriting is done because of performance reasons. this way the position of the
	 * table_onebyte does not change in memory and therefore the address can be hardcoded
	 * at compile time
	 */
	 ...
	 * If we don't catch this we could run into problems, because whenever a thread exits
	 * it's thread local storage get's cleaned up and we loose our thread local data
	 * (the struct with our tcache pointers and so on), then we either run into exceptions
	 * or just crash if assertions are turned off.
	 * fbt_commit_transaction is only needed if we use LD_PRELOAD, otherwise we can skip it
	 * (although it acts as a last guard against a bt bug)
	 */
	if (tld_key==0) {
#if defined(HIJACKCONTROL)
	  if (pthread_key_create(&tld_key, fbt_commit_transaction) != 0) {
#else
...
void fbt_start_transaction (void (*commit_function)())
{
	...
	/* set memory address of the stub of fbt_commit_transaction in the client program */
	 ...
	 */
#if defined(LDPRELOAD)
	tcache_add_entry(tld, (void*) signal, (void*) signal);
	tcache_add_entry(tld, (void*) sigaction, (void*) sigaction);
	tcache_add_entry(tld, (void*) pthread_create, (void*) pthread_create);
#endif
...
void fbt_start ()
{
	PRINT_DEBUG_FUNCTION_START("fbt_start()");
...
{
	PRINT_DEBUG_FUNCTION_START("fbt_add_translated_function_return_to_translated(original_address = %p, translated_address = %p)", original_address, translated_address);

	/* get thread local data */
	thread_local_data_t *tld = get_tld();

	tcache_add_entry(tld, original_address, trampoline_put_return_to_translated(tld, translated_address));
...
{
	PRINT_DEBUG_FUNCTION_START("fbt_set_thread_specific_app_data(data = %p)", data);
...
{
	printf("If you can read this message, this means that untranslated code was executed between fbt_start_transaction() and fbt_commit_transaction().\n");
