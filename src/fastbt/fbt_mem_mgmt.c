...
struct thread_local_data *fbt_reinit_tls(struct thread_local_data *tld) {
  ...
  *(long*)((long)(tld->mappingtable)+MAPPINGTABLE_SIZE) = 0x1; 
  ...
  /* TODO/FIX/ANALYZE: it seems that 1024 stack frames are not enough!
   * Some spec benchmarks will fail as the shadowstack keeps growing. Workaround
   * is increasing the size of the shadowstack to 1024 pages. The root cause of
   * this has still to be found as it seems that a lot of shadowstack reauths are
   * performed during run-time. */
  tld->top_of_shadowstack = fbt_lalloc(tld, 1024, MT_SHADOWSTACK);
  ...
  tld->top_of_shadowstack->return_address = (ulong_t)&tld->shadowstack_end;
