...
static void set_thread_pointer(thread_control_block *tcb) {
  ...
  tcb->sysinfo = GL_RO(_dl_sysinfo);
  
  /* Set random values for the GNU stack canary and pointer guard */
  /* TODO: We should use the random value provided by the kernel. */
  tcb->stack_guard = random_value() & 0xFFFFFF00; /* zero out LSB to harden against string function vulnerabilities */
