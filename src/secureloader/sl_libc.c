...
/* TODO: cleanup fbt*syscall calls */
...
long sl_creat(const char *filename, long mode) {
  ...
  /*asm volatile("int  $0x80"
               ...
               : "memory");
*/
_syscall2(creat, filename, mode, ret);
  return check_return(ret);
}
...
long sl_open(const char *filename, long flags) {
  ...
  /*asm volatile("int  $0x80"
               ...
               : "memory");
*/
fbt_openE(filename, flags, 0, ret);
  return check_return(ret);
}
...
void * sl_mmap(void *addr, unsigned long len, long prot, long flags, long fd,
  ...
  /* TODO:
   * Use fbt-like inlined macro syscalls. The 'g' inline assembly constraint
   * results in segfaults combined with -O3 (omit frame pointer optimizations specifically).
   */
  ...
  //asm volatile("pushl %%ebx;"
    ...
    asm volatile("pushl %%ebx;"
               ...
               "popl %%ebx"
               : "=a"(ret)
               : "0"(SYSCALL_MMAP), "gi"(offset), "gi"(fd), "gi"(flags),
               "gi"(prot), "gi"(len), "gi"(addr) 
               : "memory");
...
/* TODO: syscall handling has to be improved, if we use fbt_* macros
 * it will end up in a suicide although it might not be required. */               
//fbt_munmap(start, length, ret, "sl_munmap: failed.");
  _syscall2(munmap,start,length,ret);
  return check_return(ret);
}
...
long sl_mprotect(const void *addr, unsigned long len, long prot) {
  ...
  /*asm volatile("int  $0x80"
               ...
               : "memory");*/
fbt_mprotect(addr, len, prot, ret, "sl_mprotect: failed."); 
  return check_return(ret);
}
...
fbt_set_thread_area(u_info, ret);
  return check_return(ret);
}
...
long sl_fstat(long fd, struct kernel_stat *buf) {
  ...
  /*asm volatile("int $0x80"
               ...
               : "memory");*/
fbt_fstat(fd, buf, ret, "sl_fstat: failed.");
  return check_return(ret);
}
...
long sl_readlink(const char *path, char *buf, long bufsize) {
  ...
  /*asm volatile("int $0x80"
               ...
               : "memory");*/
fbt_readlink(path, buf, bufsize, ret);
  return check_return(ret);
}
