...
#define DUMP_CODE_IMPL(ts, instr_len, transl_len)	\
    do {						\
	pthread_mutex_lock(&dump_mutex);		\
	print_disasm_inst(dumpCodeStream, ts, instr_len);				\
	fprintf(dumpCodeStream, "--> %p (%d)\n", (char*)(ts->transl_instr-transl_len), transl_len); \
	/*fflush(dumpCodeStream);*/					\
	pthread_mutex_unlock(&dump_mutex);		\
    } while (0);

//		fprintf(dumpJmpTableStream, "0x%x 0x%x 0x%x %p --> %p\n", C_HASH_FUNCTION((int)org_addr), C_HASH_UPPER_COL((int)org_addr), C_HASH_LOWER_COL((int)org_addr), (void*)org_addr, (void*)transl_addr);	
#define DUMP_JMP_TABLE_ENTRY_IMPL(org_addr, transl_addr) \
    do {						\
	pthread_mutex_lock(&dump_mutex);		\
	fprintf(dumpJmpTableStream, "0x%x %p --> %p\n", C_HASH_FUNCTION((int)org_addr), (void*)org_addr, (void*)transl_addr); \
