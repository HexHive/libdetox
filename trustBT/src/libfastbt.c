...
#ifdef TRUST_SANDBOX
    /* trustBT: init sandbox */
    if(fbt_init_sandbox() < 0)
    {
        perror("Error initializing sandbox\n");
        exit(1);
    }
#endif
#ifdef TRUST_COUNT_MPROTECT
    mprotect_count = 0; /* set counter to 0 */
#endif
...
void fbt_cleanup()
{
/* stdout already closed, write mprotect count to a file */
#ifdef TRUST_COUNT_MPROTECT
  int fd, count;
  char buffer[256];

  if((fd = open("./cleanup_output", O_RDWR|O_CREAT|O_APPEND, S_IRUSR|S_IWUSR )) == -1) return;

  snllprintd(buffer, 256, mprotect_count);
  count = strlen(buffer);
...
void fbt_start_transaction (void (*commit_function)())
{
    ...
    */
    ...
    tcache_add_entry(tld, (void*) end_transaction_clean, (void*) end_transaction_clean);
