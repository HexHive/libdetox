...
#ifdef TRUST_COUNT_MPROTECT
int mprotect_count;
#endif

#ifdef TRUST_COUNT_MPROTECT
#  define MPROTECT_COUNT_INCREMENT mprotect_count++;
#else
#  define MPROTECT_COUNT_INCREMENT
#endif
...
void fbt_ids_lockdown(struct thread_local_data *tld)
{
    ...
    /* reset all locklist elements, just in case... */
    fbt_ids_reset_lock_list(tld);
#endif
...
/* functions for lazy memory protection */
#ifdef TRUST_MEMPROTECT
 ...
 * @param tld the thread local data pointer
 */
void unlock_locklist(struct thread_local_data *tld)
{
    /* page align address */
    void* page = (void*)((unsigned int)tld->locklist & PAGEMASK);

    /* call mprotect -> unlock pages */
 ...
 * @param tld the thread local data pointer
 */
void lock_locklist(struct thread_local_data *tld)
{
    /* page align address */
    void* page = (void*)((unsigned int)tld->locklist & PAGEMASK);

    /* call mprotect -> unlock pages */
    mprotect(page, sysconf(_SC_PAGESIZE), PROT_READ);
    MPROTECT_COUNT_INCREMENT
}

/**
 * Locks a number of pages.
 * @param tld the thread local data pointer
 * @param addr the memory address to lock
 * @param pages the number of pages to lock
 * @param is_exec the is executable flag -> PROT_EXEC
 */
void fbt_ids_lockmem(struct thread_local_data *tld, void *addr, int pages, unsigned char is_exec)
{
    /* page align address */
    void* page = (void*)((unsigned int)addr & PAGEMASK);

    /* call mprotect -> lock pages */
    mprotect(page, pages*sysconf(_SC_PAGESIZE), PROT_READ | is_exec);
    MPROTECT_COUNT_INCREMENT
}

/**
 * Relocks previously locked pages.
 * @param tld the thread local data pointer
 */
void fbt_ids_lock_list(struct thread_local_data *tld)
{
    int i;

    unlock_locklist(tld);

    for(i = 0; i < LOCKLIST_SIZE; i++)
    {
     if(tld->locklist[i].page != NULL)
     {
	/* call mprotect -> lock pages */
	mprotect(tld->locklist[i].page, tld->locklist[i].nr_pages*sysconf(_SC_PAGESIZE), PROT_READ | tld->locklist[i].execflag);
	MPROTECT_COUNT_INCREMENT
	tld->locklist[i].page = NULL;
	tld->locklist[i].nr_pages = 0;
	tld->locklist[i].execflag = 0;
     }
    }
...
/**
 * Unlocks a number of pages beginning at the lower page border of addr.
 * Stores all the parameters into the locklist to relock the pages in fbt_ids_lock_list()
 * @param tld the thread local data pointer
 * @param addr the memory address to lock
 * @param pages the number of pages to lock
 * @param is_exec the is executable flag -> PROT_EXEC
 */
void fbt_ids_unlockmem(struct thread_local_data *tld, void *addr, int pages, unsigned char is_exec)
{
   int i;
   ...
   /* iterate through locklist */
   i = 0;
   while( !(tld->locklist[i].page == page && tld->locklist[i].nr_pages >= pages) && i < LOCKLIST_SIZE ) i++;
   if(i >= LOCKLIST_SIZE)
   { /* page not already unlocked */
	/* iterate through locklist */
	i = 0;
	while(tld->locklist[i].page != NULL && i < LOCKLIST_SIZE) i++;
	if(i >= LOCKLIST_SIZE)
	{
		perror("fbt_ids_unlockmem: locklist full");
		exit(1);
	}
	else
	{ /* empty element found, set values */
    	 unlock_locklist(tld);
	 tld->locklist[i].page = page;
	 tld->locklist[i].nr_pages = pages;
	 tld->locklist[i].execflag = is_exec;
  	 lock_locklist(tld);
	}
   }
   else
   {
     /* already unlocked */
     //llprintf("%p already unlocked\n", page);
     return;
   }

    /* call mprotect -> unlock pages */
    mprotect(page, pages*sysconf(_SC_PAGESIZE), PROT_READ | PROT_WRITE | is_exec);
    MPROTECT_COUNT_INCREMENT
 ...
 * @param tld the thread local data pointer
 */
void fbt_ids_reset_lock_list(struct thread_local_data *tld)
{
    int i;
...
/**
 * Unlocks the tld struct by getting the tld pointer from get_tld().
 */
void fbt_ids_unlocktld_noarg()
{
...
/**
 * Unlocks the tld struct.
 * @param tld the thread local data pointer
 */
void fbt_ids_unlocktld(struct thread_local_data *tld)
{
 fbt_ids_unlockmem(tld, tld, 1, 0);
}

/**
 * Unlocks a page starting at page border of addr.
 * @param tld the thread local data pointer
 * @param addr the memory address to lock
 */
void fbt_ids_unlock_page(struct thread_local_data *tld, void *addr)
{
 fbt_ids_unlockmem(tld, addr, 1, PROT_EXEC);
}
