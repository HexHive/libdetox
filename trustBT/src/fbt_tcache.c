...
#endif
#ifdef TRUST_MEMPROTECT
    tld->locklist = fbt_smalloc_pers(tld, LOCKLIST_SIZE*sizeof(struct lock_list));
    if(tld->locklist == NULL)
    {
     perror("Could not allocate locklist");
     exit(1);
    }
    /* initialize locklist */
...
#ifdef TRUST_MEMPROTECT
        fbt_ids_unlockmem(tld, tld->hashtable, HASHTABLE_SIZE/sysconf(_SC_PAGESIZE), 0);
#endif
